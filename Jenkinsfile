        ...
        stage('DAST - Web Security Scan') {
            steps {
                script {
                    ...
                    sh '''
                        docker run --rm --user root -v ${WORKSPACE}:/zap/wrk \
                            $ZAP_IMAGE zap-api-scan.py -t http://$(ip -f inet -o addr show docker0 | \
                    //  awk '{print $4}' | cut -d '/' -f 1):\ 3000/auth_v1/auth/login \
                    // -f openapi -I -r report-api.html -d
                    '''
                }
